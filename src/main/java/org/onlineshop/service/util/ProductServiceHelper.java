...
public class ProductServiceHelper {
    ...
    public String resolveImageUrl(String requestedUrl) {
        String candidate = StringUtils.hasText(requestedUrl)
                ? requestedUrl.trim()
                : requiredDefaultProductImageUrl();
        return imageUrlService.validateAndNormalizeUrl(candidate);
    }

    private String requiredDefaultProductImageUrl() {
        String url = imageConfig.getProductDefault() != null
                ? imageConfig.getProductDefault().getImage()
                : null;
        if (!StringUtils.hasText(url)) {
            throw new ValidationException(
                    "Missing default product image URL. Set 'image.service.product-default.image' in application.properties");
        }
        return url.trim();
    }
}